...
function setup(checked){
 ...
 const elements={heroCallForm:form,heroCallName:input('테스트'),heroCallPhone:input('01000000000'),heroCallConsent:consent,heroCallError:error,heroCallSuccess:{hidden:true},heroCallSuccessMessage:{},heroCallRetry:input('')};
...
test('동의 시 서버에 동의 여부와 고정 문구 버전 전달',()=>{const r=setup(true);assert.equal(r.payload.privacyConsent,true);assert.equal(r.payload.privacyConsentVersion,'2026-09-23-v1');});
